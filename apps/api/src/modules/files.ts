...
router.get('/:id', async (c) => {
  ...
  try {
    ...
    return new Response(object.body, { headers: { 'Content-Type': preview ? file.content_type : 'application/octet-stream', 'Content-Disposition': `${preview ? 'inline' : 'attachment'}; filename="${filename}"`, 'Cache-Control': 'private, no-store', 'X-Content-Type-Options': 'nosniff' } });
...
router.post('/', requireRole('super_admin', 'admin', 'farm_manager', 'farm_supervisor', 'hr_officer'), requireCsrf(), async (c) => {
  ...
  if (!hasValidSignature(file, bytes)) return c.json({ error: { code: 'INVALID_FILE', message: 'The file content does not match its declared type' }, requestId: c.get('requestId') }, 422);

  const fileId = crypto.randomUUID();
  const objectKey = `master-schedule/${recordId}/${fileId}`;
  const sql = createDatabase(c.env);
  try {
    await c.env.PRIVATE_FILES.put(objectKey, file.stream(), { httpMetadata: { contentType: 'application/octet-stream', contentDisposition: 'attachment; filename="download"' } });
    ...
    await c.env.PRIVATE_FILES.delete(objectKey).catch(() => undefined);
    throw error;
  } finally { await closeDatabase(sql); }
});
