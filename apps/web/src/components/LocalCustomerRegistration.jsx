...
export default function LocalCustomerRegistration({ destination }) {
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    <div className="space-y-2"><Label className="text-label" htmlFor="local-name">Full name</Label><Input id="local-name" name="fullName" autoComplete="name" value={form.fullName} onChange={event => setForm({ ...form, fullName: event.target.value })} required minLength={2} maxLength={200} /></div>
    <div className="space-y-2"><Label className="text-label" htmlFor="local-email">Email</Label><Input id="local-email" name="email" type="email" autoComplete="email" value={form.email} onChange={event => setForm({ ...form, email: event.target.value })} required maxLength={254} /></div>
    <div className="space-y-2"><Label className="text-label" htmlFor="local-password">Password</Label><Input id="local-password" name="password" type="password" autoComplete="new-password" value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} required minLength={12} maxLength={128} aria-describedby="local-password-help" /><p id="local-password-help" className="text-xs text-muted-foreground">At least 12 characters. Use a password only for local testing.</p></div>
