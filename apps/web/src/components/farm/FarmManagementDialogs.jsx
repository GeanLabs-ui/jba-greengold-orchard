...
const emptyFarm = {
  ...
  operations_started_on: '', planting_started_on: '', description: '', notes: '',
};
...
export function FarmFormDialog({ open, onOpenChange, farm, onSubmit, saving }) {
  ...
  const submit = async (event) => {
    ...
    const saved = await onSubmit({
      ...
      soil_ph: form.soil_ph === '' ? null : Number(form.soil_ph),
    });
...
const emptyBlock = {
  ...
  size_acres: '', maturity_percent: '', forecast_yield_kg: '', fruit_fly_pressure: '', disease_rating: '',
  latitude: '', longitude: '', soil_type: '', soil_ph: '', soil_notes: '',
  operations_started_on: '', planting_started_on: '', inventory: [],
};

export function BlockFormDialog({ open, onOpenChange, block, onSubmit, saving, unallocatedAcres }) {
  ...
  const submit = async (event) => {
    ...
    const saved = await onSubmit({
      ...
      fruit_fly_pressure: form.fruit_fly_pressure.trim() || null,
      disease_rating: form.disease_rating.trim() || null,
      latitude: form.latitude === '' ? null : Number(form.latitude),
      longitude: form.longitude === '' ? null : Number(form.longitude),
      soil_ph: form.soil_ph === '' ? null : Number(form.soil_ph),
  ...
  return (
    <Dialog open={open} onOpenChange={requestClose}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader><DialogTitle>{block ? 'Edit block' : 'Add block'}</DialogTitle><DialogDescription>{block ? 'Update this block profile. Inventory changes are recorded separately to preserve history.' : `${formatNumber(unallocatedAcres)} acres are currently unallocated on this farm.`}</DialogDescription></DialogHeader>
        <form onSubmit={submit} className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Block name"><Input required value={form.name} onChange={(event) => change('name', event.target.value)} placeholder="Block A1" /></Field>
            <Field label="Block code"><Input required value={form.block_code} onChange={(event) => change('block_code', event.target.value.toUpperCase())} placeholder="A1" /></Field>
            <Field label="Early Block"><Select value={form.early_block_classification || 'not-recorded'} onValueChange={(value) => change('early_block_classification', value === 'not-recorded' ? '' : value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="not-recorded">Not recorded</SelectItem><SelectItem value="Yes">Yes</SelectItem><SelectItem value="Mid">Mid</SelectItem><SelectItem value="New">New</SelectItem></SelectContent></Select></Field>
            <Field label="Year Planted"><Input type="number" min="1900" max="2200" value={form.year_planted} onChange={(event) => change('year_planted', event.target.value)} placeholder="Not recorded" /></Field>
            <Field label="Acres"><Input type="number" min="0.01" step="0.01" value={form.size_acres} onChange={(event) => change('size_acres', event.target.value)} placeholder="Not recorded" /></Field>
            <Field label="Maturity (%)"><Input type="number" min="0" max="100" step="1" value={form.maturity_percent} onChange={(event) => change('maturity_percent', event.target.value)} /></Field>
            <Field label="Forecasted Yield (KG)"><Input type="number" min="0" step="0.01" value={form.forecast_yield_kg} onChange={(event) => change('forecast_yield_kg', event.target.value)} placeholder="Not recorded" /></Field>
            <Field label="Fruit Flies"><Input value={form.fruit_fly_pressure} onChange={(event) => change('fruit_fly_pressure', event.target.value)} placeholder="Not recorded" /></Field>
            <Field label="Disease"><Input value={form.disease_rating} onChange={(event) => change('disease_rating', event.target.value)} placeholder="Not recorded" /></Field>
            <Field label="GPS latitude"><Input type="number" min="-90" max="90" step="any" value={form.latitude ?? ''} onChange={(event) => change('latitude', event.target.value)} /></Field>
            <Field label="GPS longitude"><Input type="number" min="-180" max="180" step="any" value={form.longitude ?? ''} onChange={(event) => change('longitude', event.target.value)} /></Field>
            <Field label="Soil type"><Input value={form.soil_type || ''} onChange={(event) => change('soil_type', event.target.value)} placeholder="Sandy loam" /></Field>
            <Field label="Soil pH"><Input type="number" min="0" max="14" step="0.1" value={form.soil_ph ?? ''} onChange={(event) => change('soil_ph', event.target.value)} /></Field>
            <Field label="Operations started"><Input type="date" value={form.operations_started_on || ''} onChange={(event) => change('operations_started_on', event.target.value)} /></Field>
            <Field label="Planting started"><Input type="date" value={form.planting_started_on || ''} onChange={(event) => change('planting_started_on', event.target.value)} /></Field>
          </div>
          <Field label="Description"><Textarea value={form.description || ''} onChange={(event) => change('description', event.target.value)} rows={2} placeholder="Field description from the farm register" /></Field>
          <Field label="Soil and land notes"><Textarea value={form.soil_notes || ''} onChange={(event) => change('soil_notes', event.target.value)} rows={2} /></Field>
          ...
          <DialogFooter><Button type="button" variant="outline" onClick={() => requestClose(false)}>Cancel</Button><Button disabled={saving}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}{block ? 'Save block' : 'Create block'}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
