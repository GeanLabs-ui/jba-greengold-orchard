import { blockLabel, scopeLabel as formatFarmScope } from '@/lib/farm-scope';
...
export default function FarmFieldLogs({ farm, blocks = [], initialBlockId = 'all', lockScope = false, canCreate = false, className = 'mt-8' }) {
  ...
  return <section id="field-logs" className={`${className} scroll-mt-24 rounded-xl border bg-card shadow-sm`}>
    <div className="flex flex-col gap-4 border-b px-5 py-5 lg:flex-row lg:items-start lg:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Operational evidence</p><h2 className="mt-1 text-section-title">Field logs</h2><p className="mt-1 text-sm text-muted-foreground">Dated observations and interventions for {scopeLabel}.</p></div>
    ...
    {!lockScope ? <div className="border-b bg-muted/15 px-5 py-4"><label className="block max-w-sm text-muted-foreground text-label">View records for<select className={`${fieldClass} mt-1.5`} value={scope} onChange={(event) => setScope(event.target.value)}><option value="all">Entire farm land + all sub-blocks</option><option value="farm">Farm-wide records only</option>{blocks.map((block) => <option key={block.id} value={block.id}>{blockLabel(block)}</option>)}</select></label></div> : null}
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Add field entry</DialogTitle><DialogDescription>This record will be visible here and in Daily Activities.</DialogDescription></DialogHeader><form onSubmit={createLog} className="grid gap-4 sm:grid-cols-2">
      <label className="text-label">Log type<select name="type" className={`${fieldClass} mt-1.5`} defaultValue={type} required>{FIELD_LOG_TYPES.map((logType) => <option key={logType}>{logType}</option>)}</select></label>
      <label className="text-label">Date<Input name="entry_date" className="mt-1.5" type="date" defaultValue={today()} required /></label>
      <label className="sm:col-span-2 text-label">Farm land / sub-block<select className={`${fieldClass} mt-1.5`} value={entryScope} onChange={(event) => setEntryScope(event.target.value)} disabled={lockScope}><option value="farm">{formatFarmScope(farm.name)} · Whole farm land</option>{blocks.map((block) => <option key={block.id} value={block.id}>{blockLabel(block)}</option>)}</select></label>
      <label className="sm:col-span-2 text-label">Responsible person<Input name="owner" className="mt-1.5" minLength={2} maxLength={120} required /></label>
      <label className="sm:col-span-2 text-label">Observation / action<textarea name="notes" className={`${fieldClass} mt-1.5 min-h-28 resize-y`} minLength={3} maxLength={4000} required /></label>
      <label className="sm:col-span-2 text-label">Result / follow-up<textarea name="result" className={`${fieldClass} mt-1.5 min-h-20 resize-y`} maxLength={2000} /></label>
