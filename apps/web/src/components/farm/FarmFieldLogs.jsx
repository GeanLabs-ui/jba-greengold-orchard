...
export default function FarmFieldLogs({ farm, blocks = [], initialBlockId = 'all', lockScope = false, canCreate = false, className = 'mt-8' }) {
  ...
  const createLog = async (event) => {
    ...
    try {
      ...
      setDialogOpen(false);
      toast.success('Field log synchronized with Daily Routine Check');
    } catch (saveError) {
  ...
  return <section id="field-logs" className={`${className} scroll-mt-24 rounded-xl border bg-card shadow-sm`}>
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Add field entry</DialogTitle><DialogDescription>This record will be visible here and in Daily Routine Check.</DialogDescription></DialogHeader><form onSubmit={createLog} className="grid gap-4 sm:grid-cols-2">
