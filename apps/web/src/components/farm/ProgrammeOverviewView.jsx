  ...
  return (
    <section className={`drc-view active ${embedded ? 'drc-embedded-view' : ''}`}>
      ...
      <div className="drc-metrics">
        ...
        <Metric label="Completed" value={metrics.completed} note={`${Math.round((metrics.completed / (metrics.total || 1)) * 100)}% of Main Activities`} icon={CheckCircle2} />
      ...
      <div className="drc-dashboard-grid">
        <div className="drc-panel">
          ...
          <PanelHead title="Live Main Activities progress" copy="Select a task to review its checklist, time tracking, and updates." action={<button type="button" className="drc-btn" onClick={onOpenSchedule}>Open activities</button>} />
          <div className="drc-panel-body drc-phase-list">
            ...
            )) : <p className="text-sm text-muted-foreground">No Main Activities tasks have been created yet.</p>}
          </div>
        </div>
        <div className="drc-panel">
