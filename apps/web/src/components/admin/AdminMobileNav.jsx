...
export default function AdminMobileNav({ user, onMore }) {
  ...
  return (
    <nav className="admin-mobile-nav fixed inset-x-0 bottom-0 z-[70] grid grid-cols-5 border-t border-[#e8f5e9] bg-white px-2 pb-[max(0.4rem,env(safe-area-inset-bottom))] pt-2 md:hidden" aria-label="Quick navigation">
      {visibleDestinations.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn('flex min-h-12 flex-col items-center justify-center gap-1 rounded px-1 text-caption font-semibold transition-colors', active ? 'bg-[#2e7d32] text-white' : 'text-[#355e3b] hover:bg-[#c8e6c9] hover:text-[#123524]')}
          >
      ...
      <button type="button" onClick={onMore} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded px-1 text-caption font-semibold text-[#355e3b] transition-colors hover:bg-[#c8e6c9] hover:text-[#123524]" aria-label="Open all admin navigation">
