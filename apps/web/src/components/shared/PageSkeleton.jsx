...
function PublicChrome({ children }) {
  return <div className="skeleton-shell min-h-screen bg-[#f9fcfa]"><header className="flex h-[4.5rem] items-center border-b border-[#123524]/10 px-5 sm:px-8 lg:px-10"><div className="mx-auto flex w-full max-w-7xl items-center justify-between"><B className="h-11 w-32" /><div className="hidden gap-4 lg:flex">{Array.from({ length: 8 }, (_, i) => <B key={i} className="h-3 w-14" />)}</div><div className="flex gap-2"><B className="h-10 w-10" /><B className="hidden h-10 w-32 sm:block" /></div></div></header>{children}</div>;
}
