...
function Eyebrow({ children }) {
  return <p className="text-caption font-bold uppercase tracking-[0.14em] text-[#6b8e23]">{children}</p>;
}

function GoldBullet({ children }) {
  return (
    <li className="flex gap-2 text-sm leading-6 text-[#355e3b]">
      <CheckCircle2 className="mt-1 h-3.5 w-3.5 shrink-0 text-[#6b8e23]" />
      <span>{children}</span>
    </li>
  );
}
...
export default function Export() {
  return (
    <main className="overflow-hidden bg-[#f9fcfa] text-[#123524]">
      <section data-layout-section="hero" className="jba-public-hero relative min-h-[520px] overflow-hidden lg:min-h-[475px]">
        <img
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[#123524] via-[#123524]/80 to-[#123524]/5" />
        ...
        <div className="jba-public-hero-content relative mx-auto flex min-h-[420px] max-w-7xl items-center px-5 py-12 sm:px-8 lg:px-10">
          <div className="export-hero-copy max-w-[430px] animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <p className="mt-5 max-w-md text-body leading-6 text-white/90">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/contact?topic=export" className="platform-action inline-flex h-12 items-center rounded-md bg-[#a5d6a7] px-5 text-sm font-semibold text-[#123524] transition hover:-translate-y-0.5 hover:bg-[#a5d6a7] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">
                Start Export Inquiry <ArrowRight className="ml-3 h-4 w-4" />
              </Link>
              <Link to="/contact?topic=brochure" className="platform-action inline-flex h-12 items-center rounded-md border border-white/70 bg-black/10 px-5 text-sm font-semibold text-white backdrop-blur-sm transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">
        ...
        <div className="relative mx-auto -mt-3 max-w-7xl px-5 sm:px-8 lg:px-10">
          <div className="grid overflow-hidden rounded-t-xl border border-[#6b8e23]/50 bg-[#123524]/95 text-white shadow-2xl backdrop-blur md:grid-cols-4">
            {stats.map((stat, index) => (
              <div key={stat.label} className={`flex items-center justify-center gap-4 px-5 py-4 ${index ? 'border-t border-white/15 md:border-l md:border-t-0' : ''}`}>
                <stat.icon className="h-8 w-8 shrink-0 text-[#a5d6a7]" strokeWidth={1.6} />
                <div>
                  <p className="text-xl font-bold leading-none">{stat.value}</p>
                  <p className="mt-1 text-caption font-medium uppercase tracking-wide text-white/75">{stat.label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section data-layout-section="content" className="relative mx-auto max-w-7xl px-5 pb-8 pt-16 sm:px-8 lg:px-10">
        <div className="grid gap-10 lg:grid-cols-[280px_1fr] lg:items-end">
          <div className="relative pb-4">
            <Eyebrow>Multiple shipping modes</Eyebrow>
            <h2 className="mt-2 text-section-title">Delivered fresh.<br />By air or by sea.</h2>
            <p className="mt-4 max-w-[250px] text-sm leading-6 text-[#5f7565]">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {freightModes.map((mode) => (
              <article key={mode.title} className="group overflow-hidden rounded-lg border border-[#c8e6c9] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-xl">
                <div className="relative h-36 overflow-hidden">
                  ...
                  <span className="absolute -bottom-5 left-6 grid h-12 w-12 place-items-center rounded-full bg-[#123524] text-[#a5d6a7] ring-4 ring-white">
                    <mode.icon className="h-6 w-6" />
                  </span>
                </div>
                <div className="px-7 pb-5 pt-9">
                  <h3 className="text-card-title">{mode.title}</h3>
                  <ul className="mt-3 space-y-1">{mode.points.map((point) => <GoldBullet key={point}>{point}</GoldBullet>)}</ul>
                  <Link to="/contact?topic=export" className="mt-4 inline-flex items-center text-sm font-semibold text-[#123524] hover:text-[#6b8e23]">
      ...
      <section data-layout-section="content" className="border-y border-[#123524]/10 bg-[#f9fcfa] py-8">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-8 lg:grid-cols-[260px_1fr] lg:px-10">
          <div>
            ...
            <h2 className="mt-2 text-section-title">A global network.<br />Built on trust.</h2>
            <p className="mt-4 text-sm leading-6 text-[#5f7565]">We connect Ghana to the world with reliable sea and air freight routes.</p>
            <div className="mt-5 grid grid-cols-2 overflow-hidden rounded-lg border border-[#c8e6c9] bg-white text-sm">
              <div className="border-b border-r border-[#c8e6c9] p-3"><Globe2 className="mb-1 h-5 w-5" /><b>Europe</b><br /><span className="text-xs text-[#5f7565]">9 countries</span></div>
              <div className="border-b border-[#c8e6c9] p-3"><Anchor className="mb-1 h-5 w-5" /><b>Middle East</b><br /><span className="text-xs text-[#5f7565]">5 countries</span></div>
              <div className="border-r border-[#c8e6c9] p-3"><Route className="mb-1 h-5 w-5" /><b>Asia</b><br /><span className="text-xs text-[#5f7565]">4 countries</span></div>
              <div className="p-3"><Clock3 className="mb-1 h-5 w-5" /><b>48 hrs</b><br /><span className="text-xs text-[#5f7565]">Avg. export processing</span></div>
            </div>
            <div className="mt-3 flex items-center gap-3 rounded-lg border border-[#c8e6c9] bg-white p-3 text-sm"><Thermometer className="h-5 w-5" /><b>100%</b><span className="text-xs text-[#5f7565]">Cold-chain monitoring</span></div>
          </div>
          <figure className="group relative min-h-[350px] overflow-hidden rounded-xl bg-[#123524] shadow-lg">
      ...
      <section data-layout-section="content" className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10">
        ...
        <div className="mt-5 grid grid-cols-2 gap-y-7 sm:grid-cols-4 lg:grid-cols-8">
          {journey.map((step, index) => (
            <div key={step.title} className="relative pr-3">
              <div className="grid h-11 w-11 place-items-center rounded-full border border-[#5f7565] bg-white">
                <step.icon className="h-5 w-5 text-[#123524]" strokeWidth={1.5} />
              </div>
              {index < journey.length - 1 && <span className="absolute left-12 top-[22px] hidden h-px w-[calc(100%-3.5rem)] border-t border-dashed border-[#6b8e23] lg:block" />}
              <p className="mt-3 text-sm font-bold"><span className="mr-1">{index + 1}.</span>{step.title}</p>
              <p className="mt-1 text-xs leading-4 text-[#5f7565]">{step.copy}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {logistics.map((item) => (
            <article key={item.title} className="grid grid-cols-[42%_1fr] gap-3 rounded-lg border border-[#c8e6c9] bg-white p-3 transition hover:border-[#6b8e23] hover:shadow-md">
              ...
              <div>
                ...
                <Link to="/contact?topic=logistics" className="mt-2 inline-flex items-center text-xs font-semibold text-[#123524]">Learn more <ArrowRight className="ml-1 h-3 w-3" /></Link>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[0.82fr_1.18fr]">
          <section className="rounded-lg border border-[#c8e6c9] bg-white p-4" aria-labelledby="shipment-title">
            ...
            <div className="mt-3 grid gap-4 sm:grid-cols-[42%_1fr]">
              ...
              <dl id="shipment-title" className="grid grid-cols-[1fr_auto] content-start text-xs">
                ...
                ].map(([term, value]) => <div className="contents" key={term}><dt className="border-b border-[#e8f5e9] py-1.5 text-[#5f7565]">{term}</dt><dd className={`border-b border-[#e8f5e9] py-1.5 font-semibold ${term === 'Status' ? 'text-emerald-700' : ''}`}>{value}</dd></div>)}
              </dl>
            </div>
          </section>

          <section className="rounded-lg border border-[#c8e6c9] bg-white p-4" aria-labelledby="markets-title">
            <Eyebrow>Destination markets</Eyebrow>
            <h2 id="markets-title" className="sr-only text-section-title">Destination markets</h2>
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              {markets.map((market) => (
                <article key={market.name} className="relative min-h-36 overflow-hidden rounded-lg bg-[#123524] text-white">
                  ...
                  <div className="relative flex h-full min-h-36 flex-col justify-end p-3">
                    <h3 className="text-card-title">{market.name}</h3>
                    <p className="mt-1 text-caption leading-4 text-white/80">{market.places}</p>
      ...
      <section data-layout-section="content" className="mx-auto max-w-7xl px-5 pb-5 sm:px-8 lg:px-10">
        <div className="grid items-center gap-5 overflow-hidden rounded-xl bg-[#123524] px-6 py-5 text-white md:grid-cols-[160px_1fr_auto] lg:px-8">
          <img src={`${EXPORT_ASSET}/cta-mango.webp`} alt="Fresh cut mango" className="h-20 w-40 object-contain" loading="lazy" />
          <div>
            <h2 className="text-section-title">Ready to export the goodness of Ghanaian mangoes?</h2>
          ...
          <div className="flex flex-wrap gap-3 md:justify-end">
            <Link to="/contact?topic=export" className="platform-action inline-flex h-11 items-center rounded-md bg-[#a5d6a7] px-5 text-sm font-semibold text-[#123524] transition hover:bg-[#a5d6a7]">Start Export Inquiry <ArrowRight className="ml-3 h-4 w-4" /></Link>
            <Link to="/contact" className="platform-action inline-flex h-11 items-center rounded-md border border-white/50 px-5 text-sm font-semibold text-white transition hover:bg-white/10">Speak to our team</Link>
          </div>
        </div>
      </section>
    </main>
