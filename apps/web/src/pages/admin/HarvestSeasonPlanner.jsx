...
export default function HarvestSeasonPlanner() {
  ...
  return (
    <div className="space-y-5 pb-10">
      <section className="overflow-hidden rounded-xl border border-emerald-900/15 bg-[#f9fcfa]">
      ...
      <nav data-page-navigation className="flex flex-wrap items-center gap-1 rounded-xl border bg-card p-1.5" aria-label="Harvest planning views">
        {[
      ...
      {workspace === 'calendar' ? <section className="overflow-hidden rounded-xl border bg-card animate-in fade-in-0 slide-in-from-bottom-1 duration-200">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
          <div><h3 className="text-card-title">Harvest activity calendar</h3><p className="mt-1 text-xs text-muted-foreground">Shared with Production Calendar, company calendar subscriptions, dashboard summaries, and reminders.</p></div>
        ...
        <div className="grid xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 border-r">
            <div className="grid grid-cols-7 border-b bg-muted/35">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => <div key={day} className="py-2 text-center text-caption font-bold uppercase tracking-wider text-muted-foreground">{day}</div>)}</div>
      ...
      <Dialog open={editorOpen} onOpenChange={(open) => !saving && setEditorOpen(open)}>
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            {!editing && form.scope === 'farm' ? <label className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50/40 p-4 text-label"><input className="mt-1" type="checkbox" checked={form.apply_to_blocks} onChange={(event) => change('apply_to_blocks', event.target.checked)} /><span><b className="block text-sm">Also schedule every active sub-block</b><span className="mt-1 block text-xs leading-5 text-muted-foreground">Creates linked harvest-period and reminder records for the farm land and each active block.</span></span></label> : null}
