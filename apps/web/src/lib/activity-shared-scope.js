import { blockLabel, scopeLabel } from './farm-scope';
export function resolveActivitySharedScope(payload, blocks) {
  ...
  if (['ALL', 'A&B', 'A & B', 'FARM A & B', 'FARM A&B'].includes(scope)) {
  ...
  return {
    ...
    farm_name: sameFarm ? scopeLabel(selected[0].farm_name) : 'Farm A&B',
    block_id: selected.length === 1 ? selected[0].id : '',
    block_name: selected.map(blockLabel).join(', '),
  };
}
