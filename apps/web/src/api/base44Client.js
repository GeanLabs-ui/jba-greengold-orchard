  ...
  if (entityName === 'Product') {
    return {
      ...
      async filter(query = {}, sortBy, limit) {
        const listItems = await this.list(sortBy, limit);
        return listItems.filter((item) => matchesQuery(item, query));
      },
      ...
      async create(payload) {
        ...
        if (!res.ok) {
          ...
          throw new Error(errBody.error?.message || 'Failed to create product');
        }
        const json = await res.json();
        return json.data;
      },
