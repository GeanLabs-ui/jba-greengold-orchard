...
export type CatalogProduct = { id: string; name: string; category: string; description: string; price: number; image: string; status?: string; recordId?: string; created_date?: string };
export function mergeCatalog(records: Array<Record<string, any>>, includeDrafts = false): CatalogProduct[] {
  const products = new Map<string, CatalogProduct>(PRODUCT_CATALOG.map(product => [product.id, { ...product, status: 'published' }]));
  for (const record of records) {
    if (record.storefront !== true) continue;
    const id = record.catalog_id || record.id;
    if (!id || !record.name || !Number.isFinite(record.price) || record.price < 0) continue;
    products.set(id, { id, recordId: record.id, name: record.name, category: record.category, description: record.description, price: record.price, image: record.image, status: record.status, created_date: record.created_date });
  }
